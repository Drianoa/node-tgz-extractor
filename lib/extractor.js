 ...
 */
async function extractFromPackageLock(packageLock, options) {
    const tarballs = [];
    ...
    return await _extractTarballs(tarballsFiltered, options.directory, options.concurrency);
}
...
function _extractTarballs(tarballs, baseDirectory = './tarballs') {
    return Promise.all(tarballs.map(({url, directory}) => {
        return _ectractTarball(url, join(baseDirectory, directory))
    }))
}
